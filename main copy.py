...
# --- Configuration ---
EPISODE_NAME = "Ofir_Final_2"
PODCAST_FILE_PATH = f"PodcastsFiles/{EPISODE_NAME}.mp3"
SPEAKERS = ["Ofir", "Adi"]
# Use the latest available model that supports audio files
MODEL = "gemini-2.5-pro"
TRANSCRIPT_FILE_PATH = f"{EPISODE_NAME}_Transcription.txt"
LINKEDIN_POST_PATH = f"{EPISODE_NAME}_Linkedin.txt"
DESCRIPTION_PATH = f"{EPISODE_NAME}_Description.txt"
...
def _transcribe_segment(file_path, speakers_list, model_name):
...
def transcribe_audio(podcast_path, speakers_list, model_name, output_path):
    """Uploads an audio file and generates a transcript. Handles splitting for long files."""
    print(f"--- Starting Transcription for {podcast_path} ---")
    ...
    if duration_ms > 60 * 60 * 1000: # If > 1 hour
        ...
        for i in range(num_chunks):
            ...
            part_text = _transcribe_segment(chunk_filename, speakers_list, model_name)
            transcript_parts.append(part_text)
            
    else:
        print("Audio is under 1 hour. Transcribing directly...")
        text = _transcribe_segment(podcast_path, speakers_list, model_name)
...
transcript_path = transcribe_audio(PODCAST_FILE_PATH, SPEAKERS, MODEL, TRANSCRIPT_FILE_PATH)
...
if transcript_path:
    ...
    # generate_summary(transcript_path, SPEAKERS, MODEL, f"{EPISODE_NAME}_Summary.txt")
